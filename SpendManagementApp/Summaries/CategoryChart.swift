...
// Represents a summary of spending or income in one category
...
struct CategorySummariesPieChartView: View {
    // Chart data (loaded dynamically)
    @State var summaries: [CategorySummary] = []
    ...
    // Calculate percentage contribution for a category
    func percentage(for summary: CategorySummary) -> Double {
    ...
    func loadData() {
        ...
        self.summaries = databaseController?.fetchCategoriesBasedOnFinancialType(for: type, month: monthAndYear.month, year: monthAndYear.year) ?? []
    }
}
